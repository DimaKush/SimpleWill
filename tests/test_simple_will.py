...
def test_new_release_time(deployer, will_contract):
    """
    Test if the release time variable can be changed.
    """
    new_release_time = datetime.datetime.timestamp(datetime.datetime.now())
    will_contract.setNewReleaseTime(new_release_time, {'from': deployer})
    assert will_contract.getReleaseTime() == new_release_time


def test_set_new_beneficiary(deployer, new_beneficiary, will_contract):
    """
    Test if the beneficiary address can be changed.
    """
    will_contract.setNewBeneficiary(new_beneficiary, {'from': deployer})
    assert will_contract.getBeneficiary() == new_beneficiary
    with brownie.reverts():
        will_contract.setNewBeneficiary(new_beneficiary, {'from': new_beneficiary})


def wait_release_time(will_contract):
    time_till_release = will_contract.getReleaseTime() - datetime.datetime.timestamp(datetime.datetime.now())
    if time_till_release > 0:
        time.sleep(time_till_release)


def test_release_ERC20(will_contract, deployer, beneficiary, new_beneficiary, ERC20_token_contracts, n_contracts, value):
    ...
    for ERC20_token_contract in range(len(ERC20_token_contracts)):
        with brownie.reverts():
            will_contract.releaseERC20(ERC20_token_contracts[ERC20_token_contract], {'from': deployer})
...
def test_release_ERC721(will_contract, deployer, beneficiary, new_beneficiary, ERC721_token_contracts, n_ids, value):
    new_release_time = datetime.datetime.timestamp(datetime.datetime.now() + datetime.timedelta(seconds=value))
    will_contract.setNewReleaseTime(new_release_time, {'from': deployer})

    for ERC721_token_contract in range(len(ERC721_token_contracts)):
        with brownie.reverts():
            will_contract.releaseERC721(ERC721_token_contracts[ERC721_token_contract], list(range(n_ids)), {'from': deployer})
...
def test_release_ERC1155(will_contract, deployer, beneficiary, value, ERC1155_token_contracts, n_contracts, n_ids):
    new_release_time = datetime.datetime.timestamp(datetime.datetime.now() + datetime.timedelta(seconds=value))
    will_contract.setNewReleaseTime(new_release_time, {'from': deployer})
    for ERC1155_token_contract in ERC1155_token_contracts:
        with brownie.reverts():
            will_contract.releaseERC1155(ERC1155_token_contract, list(range(n_ids)), [value] * n_ids,  {'from': deployer})
    wait_release_time(will_contract)
    for ERC1155_token_contract in ERC1155_token_contracts:
        will_contract.releaseERC1155(ERC1155_token_contract, list(range(n_ids)), [value] * n_ids,  {'from': deployer})
